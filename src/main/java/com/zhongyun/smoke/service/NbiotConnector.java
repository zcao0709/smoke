...
import com.zhongyun.smoke.model.nbiot.NbiotFrame;
...
public class NbiotConnector extends Thread {
    ...
    private ByteBuffer sendbuffer = ByteBuffer.allocate(NbiotFrame.MAX_LEN);

    private Selector selector;
    private ServerSocketChannel server;

    private static final Logger logger = LoggerFactory.getLogger("NbiotConnector");

    @PostConstruct
    private void init() {
        logger.info("start NbiotConnector");
//        start();
    }
    ...
    private void handleKey(SelectionKey key) throws IOException {
        ...
        if (key.isAcceptable()) {
            ...
            client.register(selector, SelectionKey.OP_READ, ByteBuffer.allocate(NbiotFrame.MAX_LEN*2));
            ...
            if (count > 0) {
                NbiotFrame f = NbiotFrame.parse(recv, sensorService, config);
                if (f != null) {
